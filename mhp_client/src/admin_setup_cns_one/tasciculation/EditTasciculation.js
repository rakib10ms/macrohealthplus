...
function  EditTasciculation(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-tasciculation/${lastpart}`).then(res => {
    ...
    const updateCanalInput= (e) => {
        ...
        axios.post(`/update-tasciculation/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit Shrug The Shoulder
                        <Link to={'/tasciculation'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
