...
function EditCnsTongue(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-cnstongue/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                settongue(res.data.tongue);
    ...
    const handleInput = (e) => {
        e.persist();
        settongue({ ...tongue, [e.target.name]: e.target.value });
    }
    ...
    const updateCanalInput= (e) => {
        ...
        const data = tongue;
        axios.post(`/update-cnstongue/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
        ...
        <div className="row">
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit tongue
                        <Link to={'/cnstongue'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
                <div className="card-body">
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="form-group">
                            ...
                            <span className="text-danger">{error.tongue_name}</span>
                ...
                <div className='form-group mt-3'>
                    ...
                    <div class="form-check form-check-inline ">
                        <input className="form-check-input mt-1" type="radio"onChange={handleInput} value="single" checked={tongue.selection_criteria=='single'?true:''} name="selection_criteria"  id="inlineRadio1"/>
                        <label className="form-check-label" for="inlineRadio1"> <span style={{marginLeft:"-28px"}}>Single </span></label>
                        </div>
                        <div className="form-check form-check-inline">
                        <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={tongue.selection_criteria=='multiple'?true:''}  id="inlineRadio2"/>
