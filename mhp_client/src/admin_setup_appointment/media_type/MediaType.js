...
class MediaType extends Component {
    ...
    async componentDidMount() {
        const res = await axios.get('/media-type');
        // console.log(res);
    ...
    deleteUserType = async (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete-media-type/${id}`).then(res => {
                    if (res.data.status === 200) {
    ...
    columns = [
        ...
        {
            title: "Media Type Name", field: `media_type_name`
        ...
        {
            title: "Action", field: "patient", render: (row) => <div><Link to={`/edit-media-type/${row.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>&nbsp;
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Media Type
                                        <Link to='/add-media-type' className="btn btn-primary btn-sm float-end"> Add Media Type </Link>
                                    </h6>
                                </div>
