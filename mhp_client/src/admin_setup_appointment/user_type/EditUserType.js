...
class EditUserType extends Component {
    ...
    async componentDidMount() {
        ...
        // const user_type_id = this.props.match.params.id;
        // console.log(user_type_id);
        const res = await axios.get(`/edit-user-type/${lastpart}`);

        if (res.data.status === 200) {
            this.setState({
                user_type_name: res.data.user_type.user_type_name,
            });
    ...
    updateUserType = async (e) => {
        ...
        const res = await axios.post(`/update-user-type/${lastpart}`, this.state);
                        ...
                        <div className="col-md-9 mt-3">
                            <div className="card">
                                <div className="card-header">
                                    <h6 className="card-title">Edit User Type
                                        <Link to={'/user-type'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                    </h6>
