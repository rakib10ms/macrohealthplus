...
const EditNails = () => {
    ...
    useEffect(() => {
        axios.get(`/edit-nails/${params.id}`)
            .then(res => setNails(res.data.nails))
    }, []);
    const updateNails = (e) => {
        console.log("nails", nails)
        e.preventDefault();
        axios.post(`/update-nails/${params.id}`, nails)
            .then(res => {
                ...
                <div className="col-md-9 mt-3">
                    <div className="card">
                        <div className="card-header">
                            <h6 className="card-title"> Edit Nails
                                <Link to={'/nails'} className="btn btn-primary btn-sm float-end"> Back </Link>
                            </h6>
                        </div>
                        <div className="card-body">
                            <form id="NailsForm" onSubmit={updateNails}>
                                <div className="card-body">
