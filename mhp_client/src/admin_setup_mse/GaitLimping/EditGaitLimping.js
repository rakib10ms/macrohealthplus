...
const EditGaitLimping = () => {
    ...
    useEffect(() => {
        axios.get(`/edit-gait-limping/${params.id}`)
            .then(res => setGait(res.data.gait))
    }, []);
    const updateGait = (e) => {
        console.log("gait", gait)
        e.preventDefault();
        axios.post(`/update-gait-limping/${params.id}`, gait)
            .then(res => {
                ...
                <div className="col-md-9 mt-3">
                    <div className="card">
                        <div className="card-header">
                            <h6 className="card-title"> Edit Gait - Limping
                                <Link to={'/gait-limping'} className="btn btn-primary btn-sm float-end"> Back </Link>
                            </h6>
                        </div>
                        <div className="card-body">
                            <form id="InspectionForm" onSubmit={updateGait}>
                                <div className="card-body">
...
export default EditGaitLimping;