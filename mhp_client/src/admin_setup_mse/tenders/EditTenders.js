...
const EditTenders = () => {
    ...
    useEffect(() => {
        axios.get(`/edit-tenders/${params.id}`)
            .then(res => setTenders(res.data.tenders))
    }, []);
    const updateTenders = (e) => {
        console.log("tenders", tenders)
        e.preventDefault();
        axios.post(`/update-tenders/${params.id}`, tenders)
            .then(res => {
                ...
                <div className="col-md-9 mt-3">
                    <div className="card">
                        <div className="card-header">
                            <h6 className="card-title"> Edit Tenders
                                <Link to={'/tenders'} className="btn btn-primary btn-sm float-end"> Back </Link>
                            </h6>
                        </div>
                        <div className="card-body">
                            <form id="TendersForm" onSubmit={updateTenders}>
                                <div className="card-body">
