...
const Skin = (props) => {
    ...
    const [palpationArray, setpalpationArray] = useState([])

    const [temparatureArray, settemparatureArray] = useState([])
    const [arrangementsArray, setarrangementsArray] = useState([])
    const [AreolarArray, setAreolarArray] = useState([])

    useEffect(() => {
        axios.get(`/commonHistory-all/Dermatology`).then(res => {
            setHistory(res.data.commonHistory)
        });

        axios.get(`/skin-shape`).then(res => {
            if (res.data.status === 200) {
                setshapeArray(res.data.SkinShape);
            }
        });
        ...
        axios.get(`/palpation`).then(res => {
            if (res.data.status === 200) {
                setpalpationArray(res.data.Palpation);
            }
        });
        axios.get(`/temparature`).then(res => {
            if (res.data.status === 200) {
                settemparatureArray(res.data.Temparature);
            }

        });
        axios.get(`/arrangements`).then(res => {
            if (res.data.status === 200) {
                setarrangementsArray(res.data.Arrangements);
            }

        });

        axios.get(`/areolar`).then(res => {
            if (res.data.status === 200) {
                setAreolarArray(res.data.Areolar);
            }

        });

        axios.get(`/skin-type`).then(res => {
            if (res.data.status === 200) {
                settypeArray(res.data.skinType);
            }

        });

        axios.get(`/skin-symptoms`).then(res => {
            if (res.data.status === 200) {
                setskinSymptom(res.data.skinSymptoms);
            }
    ...
    const [siteShowAll, setsiteShowAll] = useState(false);
    const [breastShowAll, setBreastShowAll] = useState(false);
    ...
    const [scapula, setScapula] = useState(false);
    const [upperBack, setUpperBack] = useState(false);
    const [lowerBack, setLowerBack] = useState(false);
    const [chestUpper, setChestUpper] = useState(false);
    const [chestLower, setChestLower] = useState(false);
    const [arm, setArm] = useState('');
    const [forearm, setForearm] = useState('');
    const [hand, setHand] = useState('');
    const [finger, setFinger] = useState(false);
    const [interdigital, setInterdigital] = useState(false);
    const [nail, setNail] = useState(false);
    const [areolar, setAreolar] = useState('');
    const [mammary, setMammary] = useState(false);
    const [abdomen, setAbdomen] = useState(false);
    const [umbilical, setUmbilical] = useState(false);
    const [inguinal, setInguinal] = useState(false);
    const [groin, setGroin] = useState(false);
    const [buttock, setButtock] = useState(false);
    const [perineal, setPerineal] = useState(false);
    const [thigh, setThigh] = useState('');
    const [leg, setLeg] = useState('');
    const [knee, setKnee] = useState('');
    const [foot, setFoot] = useState("");
    const [interdigitalSpace, setInterdigitalSpace] = useState("");
...
const [a,setA] = useState(true);
    ...
    const [doctorImageError, setDoctorImageError] = useState();
    const handleDoctorImageUrl = (event) => {
        if (event.target.files[0].size < 4000048) {
            const images = [...doctorImage]
            images.push({
                img: URL.createObjectURL(event.target.files[0]),
                imgLink: event.target.files[0],
                bodyPart: active
            })
            setDoctorImage(images)
            setDoctorImageError(null)
        } else {
            setDoctorImageError("File size must be less than 2 mb !")
        }
    ...
    const addSymptom = (symp, index) => {
        ...
        newSymptom.push({ symp: symp, bodyPart: active });
    ...
    const popover = (
        <BPopover id="popover-basic">
            <BPopover.Body>
                <div className="symptom-popup">
                    <ul>
                        {
                            skinSymptom.map((symp, index) => <li onClick={() => addSymptom(symp, index)}>{symp.name} </li>)
                        }
    ...
    function myCanvas() {
        ...
        setCanvasImage(c.toDataURL("image/jpeg"));
    }
    function openModal() {
        setModalIsOpen(true);
        myCanvas()
    ...
    const saveSkinData = () => {
        ...
        axios.post('save-skin-examination', data).then(res => {
            console.log("Skin Res", res.data)

            for (const [key, value] of Object.entries(historyValue)) {
                const formData = new FormData();
                formData.append('patient_id', props.patient_id);
                formData.append('skin_exami_id', res.data.Skin.id);
                formData.append('history_id', `${key}`);
                formData.append('value', `${value}`);
                axios.post('save-skin-examination-history', formData).then(res => {
                    console.log("Res his", res.data)
                })
            }
            ...
            for (const [key, value] of Object.entries(type)) {
                ...
                formData.append('patient_id', props.patient_id);
                formData.append('skin_exami_id', res.data.Skin.id);
                formData.append('type_id', `${key}`);
                formData.append('value', `${value}`);
                axios.post('save-skin-examination-type', formData).then(res => {
                    console.log("Res his", res.data)
                })
            }
            ...
            doctorImage.map((item) => {
                ...
                formData.append('patient_id', props.patient_id);
                formData.append('skin_exami_id', res.data.Skin.id);
                formData.append('bodyPart_name', item.bodyPart);
                formData.append('image', item.imgLink);
                axios.post('save-skin-examination-images', formData).then(res => {
                    console.log("Res imgLink", res.data)
                })
            })
            ...
            symptoms.map((item) => {
                ...
                formData.append('skin_exami_id', res.data.Skin.id);
                formData.append('bodyPart_name', item.bodyPart);
                formData.append('symptom_id', item.symp.id);
                axios.post('save-skin-examination-syntoms', formData).then(res => {
                    console.log("Res syntoms", res.data)
                })
            })
    ...
    return (
        <div className='row'>
            <div className="history-main-header d-flex justify-content-between mb-2">
                <div>
                    <h6>Dermatology Examination</h6>
                </div>
                <div>
                    <img src={clinic9} alt="" className="img-fluid" />
                </div>
            </div>
            <div className="col-2">
                ...
                <div className="exam-bg-white p-1 mt-2">
                    <h6 className='my-1'>Rash</h6>
                    <div className="exam-bg-white mt-2 p-1">
                        <h6>Onset</h6>
                        <div className="exam-bg-white p-1">
                            <div className="d-flex justify-content-between">
                                <p className="w-75">Acute</p>
                                <div className="ms-1">
                                    <label className="switch me-1">
                                        <input
                                            name="Acute"
                                ...
                                <div className="ms-1">
                                    <label className="switch me-1">
                                        <input
                                            ...
                                            value="Jaundiced"
                                            type="checkbox"
                                            onChange={(e) => { setChange(!change); setDuration(""); setDurationType("") }}
                                            id="togBtn"
                                        />
                                        <div onClick={(e) => setAnchorEl(e.currentTarget)} className="slider round"></div>
                                        {
                                            ...
                                            >
                                                <div className="left-popup">
                                                    <div className="row">
                                                        <div className="col-6">
                                                            <input onChange={(e) => setDuration(e.target.value)} type="number" className="form-control form-control-sm" />
                                                        </div>
                                                        <div className="col-6">
                                                            <select onChange={(e) => setDurationType(e.target.value)} className="form-select form-select-sm" aria-label=".form-select-sm example">
                                                                <option defaultValue="Days">Select</option>
            ...
            <div className="col-10">
                <div className='history-main-header mb-2'>
                    <div className="d-flex">
                        ...
                        <div className="w-25">
                            <button onClick={() => setsaveLoading(false)} class="vaital-setup-btn-cancel float-end">Reset</button>
                            {
                                saveLoading ?
                                    <button className="vaital-setup-btn float-end me-2"><i className="fas fa-check-circle"></i></button>
                                    :
                                    <button onClick={saveSkinData} class="vaital-setup-btn float-end me-2">Save</button>
                ...
                <div className="col-5">
                        <>
                            ...
                            <div className=" text-center rx-one-button-group mt-2">
                                <button className='btn me-2' onClick={() => setShowBackMan(false)}> Front</button>
                                <button className='btn' onClick={() => setShowBackMan(true)}> Back</button>
                            </div>
                            {
                                active || activeBack ?
                                    <div className="row">
                                        <div className="col-6">
                                            <button className={`skin-anatomy-upload rounded-pill mt-2 ${active ? "" : "disabled"}`}>
                                                <i className="far fa-cloud-upload me-2"></i> <span>Upload Photo</span>
                                                <input type="file" onChange={handleDoctorImageUrl} />
                                            </button>
                                        </div>
                                        <div className="col-6">
                                            <OverlayTrigger trigger="click" placement="left" rootClose={true} overlay={popover}>
                                                <button className={`w-100 symptom-btn mt-2`}> Add Symptoms</button>
                                            </OverlayTrigger>
                                        </div>
                                    </div>
                                    :
                            ...
                            <div className="row">
                                <div className="col-5">
                                    {
                                        doctorImage.length > 0 &&
                                        doctorImage.map((item, i) => <div key={i} className="d-flex mt-1">
                                            {/* <h2>{item.bodyPart}</h2> */}
                                            <img className='spot-image' id="scream" src={item.img} alt="" />
                                            <i onClick={() => removeImage(i)} class="far fa-times-circle"></i>
                                        </div>)
                                    }
                                </div>
                                <div className="col-7">
                                    <div className="symptoms">
                                        {
                                            symptoms.length > 0 &&
                                            symptoms.map((symptom, index) => <div className='me-1 symptom d-inline-flex mb-2'>{symptom.name}<i onClick={() => removeSymptom(index, symptom)} className="far fa-times-circle ms-1"></i></div>)
                                        }
                                    </div>
                                </div>
                                {/* <Modal
                                ...
                                >
                                    ...
                                    <ReactPainter
                                        ...
                                        initialColor="#FF0000"
                                        lineCap="round"
                                        render={({ canvas, triggerSave, imageDownloadUrl, setColor }) => (
                                            <div>
                                                <div className="awesomeContainer">{canvas}</div>
                                                <button onClick={triggerSave}>Save</button>

                                                <a className='vaital-setup-btn' onClick={triggerSave} href={imageDownloadUrl} download>
                                                    Save
                                                </a>
                                            </div>
                                        )}
                                    />
                                    <button onClick={myCanvas} className="togB">get</button>
                                    <canvas refCanvas="canvas" className='d-none' id="myCanvas" width={640} height={425} />
                                    <img refImage="image" id="scream" src={pic} className="" />
                    ...
                    <div className="col-7">
                        <div className="row">
                            <div className="col-6">
                                <div className="show-skin-site p-1">
                                    <h6 className='my-1'>Site</h6>
                                    <div
                                        className={`g-doc-scroll ${siteShowAll ? "full-height" : "half-height"
                                            }`}
                                    >
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange1(!change1); setSite("") }}
                                                            id="togBtn"
                                                        />
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Right" type="radio" name="right" onChange={(e) => { setSite(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Right
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Left" type="radio" name="right" onChange={(e) => { setSite(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Left
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange2(!change2); setNeck("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl2(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setNeck(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setNeck(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setUpperBack(!upperBack)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setLowerBack(!lowerBack)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                        ...
                                        <div className="exam-bg-white p-1 mt-2">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Chest Lower</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Acute"
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setChestLower(!chestLower)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                        ...
                                        <div className="exam-bg-white p-1 mt-2">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Chest Upper</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Acute"
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setChestUpper(!chestUpper)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange3(!change3); setArm("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl3(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setArm(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setArm(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange4(!change4); setForearm("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl4(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setForearm(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setForearm(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange5(!change5); setHand("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl5(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setHand(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setHand(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                        ...
                                        <div className="exam-bg-white p-1 mt-2">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Interdigital</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Acute"
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setInterdigital(!interdigital)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                    ...
                                    {!siteShowAll ? (
                                        <span
                                            onClick={() => setsiteShowAll(!siteShowAll)}
                                            className="history-see-all"
                                        >
                                            Show All <i className="ms-1 far fa-angle-down"></i>
                                        </span>
                                ...
                                <div className="show-skin-site p-1 mt-2">
                                    <h6 className='my-1'>Breast</h6>
                                    <div
                                        className={`g-doc-scroll ${breastShowAll ? "full-height" : "half-height"
                                            }`}
                                    >
                                        <div className="exam-bg-white p-1 mt-1">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Areolar</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Jaundiced"
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange6(!change6); setAreolar("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl6(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    {
                                                                        AreolarArray.length > 0 &&
                                                                        AreolarArray.map((item, i) => {
                                                                            return (
                                                                                <div key={i} className="form-check ms-1">
                                                                                    <input className="form-check-input" value={item.Areolar_name} type="radio" name="right" onChange={(e) => { setAreolar(e.target.value) }} id="flexRadioDefault1" />
                                                                                    <label className="form-check-label" >
                                                                                        {item.Areolar_name}
                                                                                    </label>
                                                                                </div>
                                        ...
                                        <div className="exam-bg-white p-1 mt-2">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Sub Mammary</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Acute"
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setUmbilical(!umbilical)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setInguinal(!inguinal)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setPerineal(!perineal)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange7(!change7); setThigh("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl7(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setThigh(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setThigh(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange8(!change8); setLeg("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl8(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setLeg(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setLeg(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange9(!change9); setKnee("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl9(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setKnee(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Back" type="radio" name="right" onChange={(e) => { setKnee(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Back
                                                                        </label>
                                                                    </div>
                                                ...
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            ...
                                                            value="Jaundiced"
                                                            type="checkbox"
                                                            onChange={(e) => { setChange10(!change10); setFoot("") }}
                                                            id="togBtn"
                                                        />
                                                        <div onClick={(e) => setAnchorEl10(e.currentTarget)} className="slider round"></div>
                                                        {
                                                            ...
                                                            >
                                                                <div className="left-popup">
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Front" type="radio" name="right" onChange={(e) => { setFoot(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Front
                                                                        </label>
                                                                    </div>
                                                                    <div className="form-check ms-1">
                                                                        <input className="form-check-input" value="Under" type="radio" name="right" onChange={(e) => { setFoot(e.target.value) }} id="flexRadioDefault1" />
                                                                        <label className="form-check-label" >
                                                                            Under
                                                                        </label>
                                                                    </div>
                                        ...
                                        <div className="exam-bg-white p-1 mt-2">
                                            <div className="d-flex justify-content-between">
                                                <p className="w-75">Interdigital Space</p>
                                                <div className="ms-1">
                                                    <label className="switch me-1">
                                                        <input
                                                            name="Acute"
                                                            value="Acute"
                                                            type="checkbox"
                                                            onChange={(e) => setInterdigitalSpace(!interdigitalSpace)}
                                                            id="togBtn"
                                                        />
                                                        <div className="slider round"></div>
                                                    </label>
                                    ...
                                    {!breastShowAll ? (
                                        <span
                                            onClick={() => setBreastShowAll(!breastShowAll)}
                                            className="history-see-all"
                                        >
                                            Show All <i className="ms-1 far fa-angle-down"></i>
                                        </span>
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange17(!change17); setType({}) }}
                                                        id="togBtn"
                                                    />
                                                    <div onClick={(e) => setAnchorEl17(e.currentTarget)} className="slider round"></div>
                                                    {
                                                        ...
                                                        >
                                                            ...
                                                            <div style={{ width: "150px" }} className="left-popup">
                                                                {
                                                                    typeArray.length > 0 ?
                                                                        typeArray.map((item, i) => <div key={i} className="exam-bg-white p-1 mt-2">
                                                                            <div className="d-flex justify-content-between">
                                                                                ...
                                                                                <div className="ms-1">
                                                                                    <label className="switch me-1">
                                                                                        <input
                                                                                            name={item.id}
                                                                                            value={item.skinType_name}
                                                                                            type="checkbox"
                                                                                            onChange={(e) => setType(type[e.target.name] ? { ...type, [e.target.name]: false } : { ...type, [e.target.name]: true })}
                                                                                            id="togBtn"
                                                                                        />
                                                                                        <div className="slider round"></div>
                                                                                    </label>
                                ...
                                <div className="mt-2">
                                    <div className="exam-bg-white p-1 mt-1">
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange11(!change11); setSize1(""); setSize2(""); }}
                                                        id="togBtn"
                                                    />
                                                            ...
                                                            <div className="left-popup">
                                                                <div className="row" style={{ width: "250px" }}>
                                                                    <div className="col-6 d-flex">
                                                                        <input type="text" onChange={(e) => setSize1(e.target.value)} className="form-control form-control-sm w-75" /> <span className='ms-1'>mmX</span>
                                                                    </div>
                                                                    <div className="col-6 d-flex">
                                                                        <input type="text" onChange={(e) => setSize2(e.target.value)} className="form-control form-control-sm w-75" /> <span className='ms-1'>mm</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </Popover>
                                        ...
                                        <div className="history-popup-value">
                                            <div className="d-flex">
                                                {
                                                    size1 && <p className=''>{size1} mm X</p>
                                                }
                                                {
                                                    size2 && <p className='ms-1'>{size2} mm </p>
                                                }
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange12(!change12); setShape("") }}
                                                        id="togBtn"
                                                    />
                                                    <div onClick={(e) => setAnchorEl12(e.currentTarget)} className="slider round"></div>
                                                    {
                                                        ...
                                                        >
                                                            <div className="left-popup">
                                                                {
                                                                    shapeArray.length > 0 &&
                                                                    shapeArray.map((item, i) => {
                                                                        return (
                                                                            <div key={i} className="form-check ms-1">
                                                                                <input className="form-check-input" value={item.SkinShape_name} type="radio" name="right" onChange={(e) => { setShape(e.target.value) }} id="flexRadioDefault1" />
                                                                                <label className="form-check-label" >
                                                                                    {item.SkinShape_name}
                                                                                </label>
                                                                            </div>
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange13(!change13); setColor("") }}
                                                        id="togBtn"
                                                    />
                                                            ...
                                                            <div className="left-popup">
                                                                {
                                                                    ...
                                                                    colorArray.map((item, i) => {
                                                                        return (
                                                                            <div key={i} className="form-check ms-1">
                                                                                <input className="form-check-input" value={item.Color_name} type="radio" name="right" onChange={(e) => { setColor(e.target.value) }} id="flexRadioDefault1" />
                                                                                <label className="form-check-label" >
                                                                                    {item.Color_name}
                                                                                </label>
                                                                            </div>
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange14(!change14); setPalpation("") }}
                                                        id="togBtn"
                                                    />
                                                    <div onClick={(e) => setAnchorEl14(e.currentTarget)} className="slider round"></div>
                                                    {
                                                        ...
                                                        >
                                                            <div className="left-popup">
                                                                {
                                                                    palpationArray.length > 0 &&
                                                                    palpationArray.map((item, i) => {
                                                                        return (
                                                                            <div key={i} className="form-check ms-1">
                                                                                <input className="form-check-input" value={item.Palpation_name} type="radio" name="right" onChange={(e) => { setPalpation(e.target.value) }} id="flexRadioDefault1" />
                                                                                <label className="form-check-label" >
                                                                                    {item.Palpation_name}
                                                                                </label>
                                                                            </div>
                                    ...
                                    <div className="exam-bg-white p-1 mt-1">
                                            ...
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        ...
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange15(!change15); setTemparature("") }}
                                                        id="togBtn"
                                                    />
                                                    <div onClick={(e) => setAnchorEl15(e.currentTarget)} className="slider round"></div>
                                                    {
                                                        ...
                                                        >
                                                            <div className="left-popup">
                                                                {
                                                                    temparatureArray.length > 0 &&
                                                                    temparatureArray.map((item, i) => {
                                                                        return (
                                                                            <div key={i} className="form-check ms-1">
                                                                                <input className="form-check-input" value={item.Temparature_name} type="radio" name="right" onChange={(e) => { setTemparature(e.target.value) }} id="flexRadioDefault1" />
                                                                                <label className="form-check-label" >
                                                                                    {item.Temparature_name}
                                                                                </label>
                                                                            </div>
                                                                        )
                                        ...
                                        <div className="history-popup-value">
                                            <div className="d-flex">
                                                <p className=''>{temparature}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="exam-bg-white p-1 mt-1">
                                        <div className="d-flex justify-content-between">
                                            <p className="w-75">Arrangements</p>
                                            <div className="ms-1">
                                                <label className="switch me-1">
                                                    <input
                                                        name="Jaundiced"
                                                        value="Jaundiced"
                                                        type="checkbox"
                                                        onChange={(e) => { setChange16(!change16); setArrangements("") }}
                                                        id="togBtn"
                                                    />
                                                    <div onClick={(e) => setAnchorEl16(e.currentTarget)} className="slider round"></div>
                                                    {
                                                        ...
                                                        >
                                                            <div className="left-popup">
                                                                {
                                                                    arrangementsArray.length > 0 &&
                                                                    arrangementsArray.map((item, i) => {
                                                                        return (
                                                                            <div key={i} className="form-check ms-1">
                                                                                <input className="form-check-input" value={item.Arrangements_name} type="radio" name="right" onChange={(e) => { setArrangements(e.target.value) }} id="flexRadioDefault1" />
                                                                                <label className="form-check-label" >
                                                                                    {item.Arrangements_name}
                                                                                </label>
                                                                            </div>
                                                                        )
