...
function EditSymptoms(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-symptoms/${lastpart}`).then(res => {
    ...
    const updateCanalInput= (e) => {
        ...
        const data = symptoms;
        axios.post(`/update-symptoms/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit Symptoms
                        <Link to={'/symptoms'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
                <div className="card-body">
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="form-group">
                            ...
                            <input type="text"  value={symptoms.symptoms_name} className="form-control" name="symptoms_name" onChange={handleInput} />
                            <span className="text-danger">{error.symptoms_name}</span>
                        ...
                        <select id="inputState" onChange={handleInput} value={symptoms.status_id} name="status_id"
                ...
                <div className='form-group mt-3'>
                    ...
                    <div class="form-check form-check-inline ">
                        <input className="form-check-input mt-1" type="radio"onChange={handleInput} value="single" checked={symptoms.selection_criteria=='single'?true:''} name="selection_criteria"  id="inlineRadio1"/>
                        <label className="form-check-label" for="inlineRadio1"> <span style={{marginLeft:"-28px"}}>Single </span></label>
                        </div>
                        <div className="form-check form-check-inline">
                        <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={symptoms.selection_criteria=='multiple'?true:''}  id="inlineRadio2"/>
