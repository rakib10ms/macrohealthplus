...
function EditBreathSound(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-breathsound/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                console.log(res.data);
                setbreathsound(res.data.breathsound);
                setPicture(res.data.breathsound.image);
    ...
    const handleInput = (e) => {
        e.persist();
        setbreathsound({ ...breathsound, [e.target.name]: e.target.value });
    }
    ...
    const updateCanalInput= (e) => {
        ...
        formData.append('breathsound_name',  breathsound.breathsound_name);
        formData.append('status_id', breathsound.status_id);
        formData.append('selection_criteria', breathsound.selection_criteria);
    
            axios.post(`/update-breathsound/${lastpart}`, formData).then(res => {
            if (res.data.status === 200) {
        ...
        <div className="row">
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit breathsound
                        <Link to={'/breathsound'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
                <div className="card-body">
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="form-group">
                            ...
                            <input type="text"  value={breathsound.breathsound_name} className="form-control" name="breathsound_name" onChange={handleInput} />
                            <span className="text-danger">{error.breathsound_name}</span>
                        ...
                        <select id="inputState" onChange={handleInput} value={breathsound.status_id} name="status_id"
                ...
                <div className='form-group mt-3'>
                    ...
                    <div class="form-check form-check-inline ">
                        <input className="form-check-input mt-1" type="radio"onChange={handleInput} value="single" checked={breathsound.selection_criteria=='single'?true:''} name="selection_criteria"  id="inlineRadio1"/>
                        <label className="form-check-label" for="inlineRadio1"> <span style={{marginLeft:"-28px"}}>Single </span></label>
                        </div>
                        <div className="form-check form-check-inline">
                        <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={breathsound.selection_criteria=='multiple'?true:''}  id="inlineRadio2"/>
            ...
            <div className="form-group mt-3">
                                                    <label htmlFor="inputEmail4"
                                                        className="form-label col-form-label col-form-label-sm"> Breath Sound Image </label>
                                                    ...
                                                    {imageUrl == null ? <img src={`${global.img_url}/images/files/breathsound/${picture}`} className="doctorImageUrlPreview" /> :
...
export default EditBreathSound
