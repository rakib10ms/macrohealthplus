...
function EditMood(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-mood/${lastpart}`).then(res => {
    ...
    const updateCanalInput= (e) => {
        ...
        const data = mood;
        axios.post(`/update-mood/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
        ...
        <div className="row">
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit Mood
                        <Link to={'/mood'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
                <div className="card-body">
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="form-group">
                            ...
                            <input type="text"  value={mood.mood_name} className="form-control" name="mood_name" onChange={handleInput} />
                            <span className="text-danger">{error.mood_name}</span>
                        ...
                        <select id="inputState" onChange={handleInput} value={mood.status_id} name="status_id"
                ...
                <div className='form-group mt-3'>
                    ...
                    <div class="form-check form-check-inline ">
                        <input className="form-check-input mt-1" type="radio"onChange={handleInput} value="single" checked={mood.selection_criteria=='single'?true:''} name="selection_criteria"  id="inlineRadio1"/>
                        <label className="form-check-label" for="inlineRadio1"> <span style={{marginLeft:"-28px"}}>Single </span></label>
                        </div>
                        <div className="form-check form-check-inline">
                        <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={mood.selection_criteria=='multiple'?true:''}  id="inlineRadio2"/>
