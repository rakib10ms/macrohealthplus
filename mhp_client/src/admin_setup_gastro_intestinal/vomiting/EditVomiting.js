...
function EditVomiting(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-vomiting/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                console.log(res.data);
                setVomiting(res.data.Vomiting);
                // setPicture(res.data.Vomiting.image);
    ...
    const handleInput = (e) => {
        e.persist();
        setVomiting({ ...Vomiting, [e.target.name]: e.target.value });
    }

    const updateVomitingInput = (e) => {
        e.preventDefault();
        ...
        axios.post(`/update-vomiting/${lastpart}`, formData).then(res => {
            if (res.data.status === 200) {
                    ...
                    <div className="col-md-9 mt-3">
                        <div className="card">
                            <div className="card-header">
                                <h6 className="card-title"> Edit Vomiting
                                    <Link to={'/vomiting'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                </h6>
                            </div>
                            <div className="card-body">
                                <form id="VomitingForm" onSubmit={updateVomitingInput}>
                                    <div className="card-body">
                                        ...
                                        <div className="col-md-12">
                                            ...
                                            <div className="form-group">
                                                ...
                                                <span className="text-danger">{error.Vomiting_name}</span>
                                            </div>
