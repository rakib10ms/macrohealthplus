...
function EditFrequency(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-frequency/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                console.log(res.data);
                setFrequency(res.data.Frequency);
                // setPicture(res.data.Frequency.image);
    ...
    const handleInput = (e) => {
        e.persist();
        setFrequency({ ...Frequency, [e.target.name]: e.target.value });
    }

    const updateFrequencyInput = (e) => {
        e.preventDefault();
        ...
        axios.post(`/update-frequency/${lastpart}`, formData).then(res => {
            if (res.data.status === 200) {
                    ...
                    <div className="col-md-9 mt-3">
                        <div className="card">
                            <div className="card-header">
                                <h6 className="card-title"> Edit Frequency
                                    <Link to={'/frequency'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                </h6>
                            </div>
                            <div className="card-body">
                                <form id="FrequencyForm" onSubmit={updateFrequencyInput}>
                                    <div className="card-body">
                                        ...
                                        <div className="col-md-12">
                                            ...
                                            <div className="form-group">
                                                ...
                                                <span className="text-danger">{error.Frequency_name}</span>
                                            </div>
