...
function EditNonTenderTenderness(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-non-tender-tenderness/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                console.log(res.data);
                setnontendertenderness(res.data.nontendertenderness);
                // setPicture(res.data.nontendertenderness.image);
    ...
    const handleInput = (e) => {
        e.persist();
        setnontendertenderness({ ...nontendertenderness, [e.target.name]: e.target.value });
    }

    const updateNonTenderTendernessInput = (e) => {
        e.preventDefault();
        ...
        axios.post(`/update-non-tender-tenderness/${lastpart}`, formData).then(res => {
            if (res.data.status === 200) {
                    ...
                    <div className="col-md-9 mt-3">
                        <div className="card">
                            <div className="card-header">
                                <h6 className="card-title"> Edit Non-Tender Tenderness
                                    <Link to={'/non-tender-tenderness'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                </h6>
                            </div>
                            <div className="card-body">
                                <form id="NonTenderTendernessForm" onSubmit={updateNonTenderTendernessInput}>
                                    <div className="card-body">
                                        ...
                                        <div className="col-md-12">
                                            ...
                                            <div className="form-group">
                                                ...
                                                <span className="text-danger">{error.nontendertenderness_name}</span>
                                            </div>
...
export default EditNonTenderTenderness;
