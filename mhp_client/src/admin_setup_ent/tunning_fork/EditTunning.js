...
function EditTunning(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-tunning/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                settunning(res.data.tunning);
    ...
    const handleInput = (e) => {
        e.persist();
        settunning({ ...tunning, [e.target.name]: e.target.value });
    }

    const updatetunning= (e) => {
        e.preventDefault();
        ...
        const data = tunning;
        axios.post(`/update-tunning/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
        ...
        <div className="row">
            ...
            <div className="card">
                <div className="card-header">
                    <h6 className="card-title"> Edit Tunning Fork
                        <Link to={'/tunning-fork'} className="btn btn-primary btn-sm float-end"> Back </Link>
                    </h6>
                </div>
                <div className="card-body">
                    <form id="EartunningForm" onSubmit={updatetunning}>
                        <div className="card-body">
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="form-group">
                            ...
                            <span className="text-danger">{error.tunning_name}</span>
                ...
                <div className='form-group mt-3'>
                    ...
                    <div class="form-check form-check-inline ">
                        <input className="form-check-input mt-1" type="radio"onChange={handleInput} value="single" checked={tunning.selection_criteria=='single'?true:''} name="selection_criteria"  id="inlineRadio1"/>
                        <label className="form-check-label" for="inlineRadio1"> <span style={{marginLeft:"-28px"}}>Single </span></label>
                        </div>
                        <div className="form-check form-check-inline">
                        <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={tunning.selection_criteria=='multiple'?true:''}  id="inlineRadio2"/>
