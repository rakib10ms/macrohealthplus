...
function EditMastoid(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-mastoid/${lastpart}`).then(res => {

            if (res.data.status === 200) {
                setmastoid(res.data.mastoid);
    ...
    const handleInput = (e) => {
        e.persist();
        setmastoid({ ...mastoid, [e.target.name]: e.target.value });
    }

    const updatemastoid = (e) => {
        e.preventDefault();
        ...
        const data = mastoid;
        axios.post(`/update-mastoid/${lastpart}`, data).then(res => {
            if (res.data.status === 200) {
                    ...
                    <div className="col-md-9 mt-3">
                        <div className="card">
                            <div className="card-header">
                                <h6 className="card-title"> Edit Mastoid
                                    <Link to={'/mastoid'} className="btn btn-primary btn-sm float-end"> Back </Link>
                                </h6>
                            </div>
                            <div className="card-body">
                                <form id="EarmastoidForm" onSubmit={updatemastoid}>
                                    <div className="card-body">
                                        ...
                                        <div className="col-md-12">
                                            ...
                                            <div className="form-group">
                                                ...
                                                <span className="text-danger">{error.mastoid_name}</span>
                                            ...
                                            <div className='form-group mt-3'>
                                                ...
                                                <div class="form-check form-check-inline ">
                                                    <input className="form-check-input mt-1" type="radio" onChange={handleInput} value="single" checked={mastoid.selection_criteria == 'single' ? true : ''} name="selection_criteria" id="inlineRadio1" />
                                                    <label className="form-check-label" for="inlineRadio1"> <span style={{ marginLeft: "-28px" }}>Single </span></label>
                                                </div>
                                                <div className="form-check form-check-inline">
                                                    <input className="form-check-input mt-1" type="radio" onChange={handleInput} name="selection_criteria" value="multiple" checked={mastoid.selection_criteria == 'multiple' ? true : ''} id="inlineRadio2" />
